...
function transformChebInPlace1D(coeffs,alpha,beta, macheps=type(2)^-(precision-1))
    ...
    # arr2 = zeros(type, last_dim_length)
    # arr3 = zeros(type, last_dim_length)
    C = zeros(type, last_dim_length,last_dim_length)
    #The first column of the transformation matrix C. Since T_0(alpha*x + beta) = T_0(x) = 1 has 1 in the top entry and 0's elsewhere.
    C[1,1] = type(1)
    C[2,1] = type(beta)
    C[2,2] = type(alpha)
    ...
    for k in 3:last_dim_length
        C[k,1] = type(2)*beta*C[k-1,1] - C[k-2,1] + alpha*C[k-1,2]
        C[k,2] = type(2)*beta*C[k-1,2] - C[k-2,2] + alpha*C[k-1,3] + type(2)*alpha*C[k-1,1]
        for i in 3:j-1
            C[k,i] = type(2)*beta*C[k-1,i] - C[k-2,i] + alpha*C[k-1,i+1] + alpha*C[k-1,i-1]
        end
        if abs(alpha*C[k-1,j-1]) > macheps
